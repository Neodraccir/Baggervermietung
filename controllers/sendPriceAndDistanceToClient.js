module.exports = (app) => {
  const appRoot = require("app-root-path"),
    ...
    writeTMPrequest = async function (userID, requestData) {
      try {
        await fsp.writeFile(`${appRoot}/db//tmp/${userID}.json`, requestData, {
          flags: "w",
        });
        console.log(`writing >> ${requestData}`);
        console.log(
          `#SPADTC: ${userID}.json was sucessfully created in: "{root}/db/tmp/"`
        );
      } catch (err) {
        console.log(err);
      }
  ...
  app.post("/placeReq", (req, res) => {
    ...
    let mapsAPIDATA = require(`${appRoot}/config/mapsAPIData.json`),
      ...
      extraBuckets = req.body.extraBuckets,
    ...
    console.log("#SPADTC: Recieved this Data from Client:")
    ...
    distanceCalc(origin, placeReq, api_key)
      .then((dist) => {
        let distance = Math.round(dist?.[0] / 1000),
          ...
          travelTime = dist[1],
          getGlobalStrings =
            require(`${appRoot}/lib/global/getGlobalStrings.js`)(globalThis), //includes variable "rentalCost" & "deliveryCost"
          maxDistance = Math.floor(deliveryPrizes.length - 1);

        if (distance > maxDistance) {
          inclDelivery = false;
        }
        if (inclDelivery == "false") {
          prize.deliveryCost = 0;
          prize.fullCost = prize.rentalCost;
        }
        if (isNaN(prize.deliveryCost)) {
          prize.fullCost = prize.rentalCost;
        }

        let response = `{"dist":"${distance}","fullCost":"${
            prize.fullCost
          }","deliveryCost":"${prize.deliveryCost}","rentalCost":"${
            prize.rentalCost
          }", "maxDistance":"${maxDistance}", "isInclDelivery":"${inclDelivery}", "isEnterprise":"${isBusiness}","company":"${company}","user_id":"${user_id}","order_id":"${order_id}", "amountOfDays":"${amountOfDays}", "dateStart":"${convertDate(
            dateStart
          )}", "dateEnd":"${convertDate(
            dateEnd
          )}", "travelTime":"${travelTime}", "place":"${place}", "msg":"${msg}", "phoneNr":"${phoneNr}" }`,
          tmpRequest = `{"order_id":"${order_id}","user_id":"${user_id}","lastName":"${lastName}","firstName":"${firstName}","distance":"${distance}","fullCost":"${
            prize.fullCost
          }","deliveryCost":"${prize.deliveryCost}","rentalCost":"${
            prize.rentalCost
          }", "maxDistance":"${maxDistance}", "isInclDelivery":"${inclDelivery}", "isEnterprise":"${isBusiness}","company":"${company}", "amountOfDays":"${amountOfDays}", "dateStart":"${convertDate(
            dateStart
          )}", "dateEnd":"${convertDate(
            dateEnd
          )}", "travelTime":"${travelTime}","place":"${place}", "msg":"${msg}", "phoneNr":"${phoneNr}" }`;
