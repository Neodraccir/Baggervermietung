...
function inputCreator(inputsObject, form) {
  ...
  values.forEach((element, index) => {
    let inputElement = form[keys[index]],
      ...
      returnStringOr = (potentialString, alternative) =>
        typeof potentialString === "string" ? potentialString : alternative,
      newInputElement = function (required = true, textarea = false) {
        let spanElement = new ElementWithAttributes(
          "span",
          { attribute: "class", value: keys[index] },
          parent ? parent : form
        );

        spanElement.innerHTML = returnStringOr(
          element,
        ...
        let attributesAndValuesArray = [
          ...
          thisReturnsDuplicateAttributeIfExisting = (currentObject) => {
            for (let i = 0; i < attributesAndValuesArray.length; i++) {
              if (
                attributesAndValuesArray[i].attribute ===
            ...
            if (arg.length >= 3) {
              for (let i = 2; i < arg.length; i++) {
                if (arg[i]?.attribute && arg[i]?.value) {
                  let duplicate = thisReturnsDuplicateAttributeIfExisting(
                    arg[i]
                  );
                  if (duplicate) {
                    attributesAndValuesArray[
                      attributesAndValuesArray.indexOf(duplicate)
                    ] = arg[i];
                  } else {
                ...
                if (!arg[i]?.attribute && !arg[i]?.value) {
                  logThis(
                    `Object ${arg[i]} has no 'value' and no 'attribute' property`
                  );
                }
              }
            }
        ...
        inputElement = new ElementWithAttributes(
      ...
      newSwitchElement = () => {
        let newCheckboxSwitch = new ElementWithAttributes(
          "input",
          [
        ...
        let newSpanForSwitch = new ElementWithAttributes(
          "span",
          { attribute: "class", value: "slider round" },
          parent ? parent : form
        );
        let aboveButtonDiv = new ElementWithAttributes(
          "div",
          [
        ...
        let belowButtonDiv = new ElementWithAttributes(
          "div",
          [
    ...
    if (typeof element === "object") {
      switch (Object.keys(element)[0]) {
        ...
        case "textarea":
          newInputElement(false, "textarea");
          break;
        ...
        case "buckets":
          let title = new ElementWithAttributes(
            "span",
            { attribute: "class", value: "extraBuckets" },
            parent ? parent : form
          );
          title.innerText = element.title;
          element.buckets.forEach((bucket) => {
            let bucketDiv = new ElementWithAttributes(
                "div",
                { attribute: "class", value: "bucket" },
                parent ? parent : form
              ),
              bucketHeadline = new ElementWithAttributes(
                "h3",
                { attribute: "class", value: "bucketHeadline" },
                bucketDiv
              ),
              bucketSize = new ElementWithAttributes(
                "span",
                { attribute: "class", value: "bucketSize" },
                bucketDiv
              ),
              bucketCheckbox = new ElementWithAttributes(
                "input",
                [
                  { attribute: "class", value: "extraBucket" },
                  { attribute: "name", value: bucket.name },
                  { attribute: "value", value: false },
                  { attribute: "type", value: "checkbox" },
                ],
                bucketDiv
              );
            bucketHeadline.innerHTML = bucket.name;
            bucketSize.innerHTML = bucket.size;
            bucketDiv.classList.add("unselected")
            bucketDiv.onclick = () => {
              bucketCheckbox.checked = !bucketCheckbox.checked;
             // postPlaceReq();
            };
          });
