...
function buildAfterSentMessageBox(orderRequestMessageStatus) {
  body.afterSentMessageBox = addGlobalVar({
    afterSentMessageBox: new ElementWithAttributes(
      "div",
  ...
  body.afterSentMessageBox.x = new ElementWithAttributes(
    "div",
    { attribute: "class", value: "X" },
    body.afterSentMessageBox
  );

  body.afterSentMessageBox.x.content = new ElementWithAttributes(
    "div",
    { attribute: "class", value: "X" },
    body.afterSentMessageBox.x
  );

  body.afterSentMessageBox.sentBoxContent = new ElementWithAttributes(
    "div",
  ...
  document.addEventListener("click", (event) => {
    if (
      event.target ==
      document.querySelector("#afterSentMessageBox > div.X > div")
    ) {
      document
        .querySelectorAll("#afterSentMessageBox")
        .forEach((e) => e.remove());
    }
  });

  if (orderRequestMessageStatus === "sent") {
    body.afterSentMessageBox.sentBoxContent.innerHTML =
      "Anfrage wurde erfoglreich versendet.\nJemand wird sich in Kürze bei Ihnen melden. ";
  }
