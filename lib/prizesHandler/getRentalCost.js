module.exports = (timeObject, isBusiness)=>{
    ...
    //dependencies and variables   
        let appRoot                       = require('app-root-path'),
            ...
            amountOfDays                  = require(`${appRoot}/lib/prizesHandler/getAmountOfDays.js`)(startDate, endDate);
        ...
        if(amountOfDays<0){return console.log("#GRC: ERROR: amountOfDays < 0")}

    //functions
        function determinePriceConditionsFor(timespan){
                   let priceObject = returnArrayObjectOrNextEntry(timespan, rentalPrizes, true);
                return priceObject;
              }
        ...
        function getWeekdaySpecialPrice(rentalPrizesObject){
            ...
            let specialStartWeekday = convertWeekdayStringToNumber(rentalPrizesObject?.[fromTo]?.[0]),
                specialEndWeekday   = convertWeekdayStringToNumber(rentalPrizesObject?.[fromTo]?.[1]),
        ...
        function getRentalPriceFor(timeObject, isBusiness){
            ...
            if(thereAreSpecialWeekdayPrices){
                ...
                let specialWeekdayPrize = getWeekdaySpecialPriceOrReturnFalse(rentalPrizesObject);
                ...
                if(specialWeekdayPrize){return specialWeekdayPrize}
                };
            ...
            if(isBusiness){
                ...
                rentalPrize = calculatePrize(rentalPrizesObject?.business, rentalPrizesObject?.minDays, amountOfDays)
                };

            if(!isBusiness){

                rentalPrize = calculatePrize(rentalPrizesObject?.private, rentalPrizesObject?.minDays, amountOfDays)
                };
